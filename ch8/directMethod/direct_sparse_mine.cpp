...
class VertexPose : public g2o::BaseVertex<6, Sophus::SE3d> { // Use Sophus::SE3 instead of g2o::SE3
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
...
void extractFeatures(cv::Mat& gray, cv::Mat& depth, vector<Measurement>& measurements, int mode) {
    ...
    switch (mode)
    {
        ...
        case 1: {
            int grid_size = 10; // grid size is 40*40
            ...
            for(int y=0;y<rows_grid;y++) {
                for(int x=0;x<cols_grid;x++) {
                    ...
                    for (int j = 0; j < grid_size; j++) { // row
                        for (int i = 0; i < grid_size; i++) { // col
                            Eigen::Vector2d delta (
                                gray.ptr<uchar>(j+grid_size*y)[i+grid_size*x+2] - gray.ptr<uchar>(j+grid_size*y)[i+grid_size*x-2], 
                                gray.ptr<uchar>(j+grid_size*y+2)[i+grid_size*x] - gray.ptr<uchar>(j+grid_size*y-2)[i+grid_size*x]
                            );
                            int g = delta.norm();
                            if ( g < 20 )
                                continue;
                    ...
                    int grad_th_cnt = grad_cnt[0]*0.5;
                    int th = 0;
                    int max_num = 30;
                    for (int i = 99; i > 20; i--) {
                        grad_th_cnt -= grad_cnt[i];
                        max_num -= grad_cnt[i];
                        if(y==15 && x==19) std::cout<<"max_num = "<<max_num<<" "<<i<<" "<<grad_th_cnt<<" "<<grad_cnt[i]<<std::endl;
                    ...
                    if(y==15 && x==19) cout << "th = " << th << " " << grad_cnt[0] << " "<< grad_cnt[1] << " "<< grad_cnt[2] << endl;
                    if (th<20) th = 20;
                    for (int j = 1; j < grid_size-1; j++) { // row
                        for (int i = 1; i < grid_size-1; i++) { // col
                            ...
                            if ( g > th) {
                                ...
                                if ( float ( d ) /depth_scale < 0.03 || float ( d ) /depth_scale > 60)
                                    continue;
...
int main ( int argc, char** argv )
{
    ...
    // std::string timestamp_path = "/home/zh/data/img/rgbd_timestamp.txt";
    ...
    gray = cv::imread(rgbd_dataset_path_+"/syn/rgb_recorded/1660245762.702137.png", cv::IMREAD_GRAYSCALE);
    ...
    // cv::waitKey(0);
    extractFeatures(gray, depth, measurements, 1);
    ...
    pose->setEstimate ( Sophus::SE3d ( Tcw.rotation(), Tcw.translation() ) );
    pose->setId ( 0 );
    ...
    cout << "pose = " << endl << pose->estimate().rotationMatrix() << endl << pose->estimate().translation().transpose() << endl;
    ...
    Tcw.rotate ( pose->estimate().rotationMatrix() );
